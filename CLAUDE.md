# Prototype A2 — import review without field flags

**SmartReturn ProtoA2** is a sibling of ProtoA: same import review experience (output forms, doc verify banner, coach tips, summary layout) but **without orange Phase 1 field flags**. Progress is driven by verifying source documents only. No Phase 2 / AI diagnostics.
...
- **Start the preview:** Use `/preview` to see your work at `http://localhost:5176`
...
Each page has its own file in `src/pages/` and its own CSS module in `src/styles/`. Pages are accessible at `http://localhost:5176/#/[page-path]`. Skills handle creating new pages and wiring them into the router.
