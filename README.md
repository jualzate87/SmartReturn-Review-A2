...
Sibling of SmartReturn ProtoA. Same import review UX without orange field flags; document verification drives progress. Runs on port **5176**.
...
Your prototype will be running at **http://localhost:5176**. Changes you make are reflected instantly in the browser.
...
| Command | Description |
|---------|-------------|
| `yarn dev` | Start the dev server at http://localhost:5176 |
...
| Dev server won't start | Try `yarn install` again, then `yarn dev`. Make sure port 5176 isn't already in use. |
